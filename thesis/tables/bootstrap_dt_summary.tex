\begin{table}
\caption{Within-sample DT reproducibility versus observed between-split $\rho_S^{\mathrm{DT}}$. For each sub-sample the DT surrogate was re-fitted on identical data with $N=10$ random seeds, yielding 45 pairwise Spearman~$\rho$ values per sub-sample. Within (mean) is the average of those means across the two sub-samples of each split. Gap $=$ Within $-$ Between. Classification: $>0.10$ genuine signal; $0.03$ to $0.10$ small effect; $\leq 0.03$ indistinguishable from refit noise. Raw pairwise values are in \path{data/artifacts/l5b15/bootstrap_dt_routes.json}.}
